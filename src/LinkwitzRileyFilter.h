...
namespace Constants {
    ...
    template<typename T>
    constexpr T Q_LR4 = static_cast<T>(0.7071067811865476);
    
    // LR8 (48dB/oct) = 4th order Butterworth squared: Q1 = 0.5412, Q2 = 1.3065
    // Q1 = 1 / (2 * cos(pi/8)) = 0.5412 (for stages 1,2)
    template<typename T>
    constexpr T Q_LR8_1 = static_cast<T>(0.5411961001461971);
    
    // Q2 = 1 / (2 * cos(3*pi/8)) = 1.3065 (for stages 3,4)
    template<typename T>
    constexpr T Q_LR8_2 = static_cast<T>(1.3065629648763766);
 ...
 */
template<typename SampleType>
BiquadCoeffs<SampleType> calcBiquadCoeffsWithQ(FilterType type, SampleType freq, SampleType sampleRate, SampleType Q) {
    const SampleType omega = static_cast<SampleType>(2) * Constants::PI<SampleType> * freq / sampleRate;
...
public:
    ...
    void setParams(FilterType type, Slope slope, SampleType freq, SampleType sampleRate) {
        ...
        if (type == FilterType::AllPass) {
            ...
            if (slope == Slope::DB24) {
                ...
                m_coeffsList[0] = calcBiquadCoeffsWithQ<SampleType>(FilterType::AllPass, freq, sampleRate, Constants::Q_LR4<SampleType>);
            } else {
                // LR8 LP+HP = 4th order allpass with Q1 = 0.5412, Q2 = 1.3065
                m_coeffsList[0] = calcBiquadCoeffsWithQ<SampleType>(FilterType::AllPass, freq, sampleRate, Constants::Q_LR8_1<SampleType>);
                m_coeffsList[1] = calcBiquadCoeffsWithQ<SampleType>(FilterType::AllPass, freq, sampleRate, Constants::Q_LR8_2<SampleType>);
            }
        } else if (slope == Slope::DB24) {
            // LR4: 2 stages, both with Q = 0.7071
            auto c = calcBiquadCoeffsWithQ<SampleType>(type, freq, sampleRate, Constants::Q_LR4<SampleType>);
            m_coeffsList[0] = c;
            m_coeffsList[1] = c;
        } else {
            // LR8: 4 stages - stages 1,2 with Q1, stages 3,4 with Q2
            auto c1 = calcBiquadCoeffsWithQ<SampleType>(type, freq, sampleRate, Constants::Q_LR8_1<SampleType>);
            auto c2 = calcBiquadCoeffsWithQ<SampleType>(type, freq, sampleRate, Constants::Q_LR8_2<SampleType>);
            m_coeffsList[0] = c1;
            m_coeffsList[1] = c1;
            m_coeffsList[2] = c2;
            m_coeffsList[3] = c2;
    ...
    Slope getSlope() const { return m_slope; }
    SampleType getFreq() const { return m_freq; }
    SampleType getSampleRate() const { return m_sampleRate; }
    size_t getNumStages() const { return m_numStages; }
...
public:
    ...
    StereoLinkwitzRileyFilter(FilterType type, Slope slope, SampleType freq, SampleType sampleRate)
        : m_left(type, slope, freq, sampleRate)
        , m_right(type, slope, freq, sampleRate) {
    }
    ...
    LinkwitzRileyFilter<SampleType>& getLeft() { return m_left; }
    LinkwitzRileyFilter<SampleType>& getRight() { return m_right; }
...
public:
    ...
    CrossOver(Slope slope, SampleType freq, SampleType sampleRate)
        : m_slope(slope)
        , m_lp(FilterType::LowPass, slope, freq, sampleRate)
     ...
     */
    void processSample(SampleType inL, SampleType inR,
                       SampleType& lpL, SampleType& lpR,
                       SampleType& hpL, SampleType& hpR) {
        m_lp.processSample(inL, inR, lpL, lpR);
        m_hp.processSample(inL, inR, hpL, hpR);
    }
    
    StereoLinkwitzRileyFilter<SampleType>& getLowPass() { return m_lp; }
    StereoLinkwitzRileyFilter<SampleType>& getHighPass() { return m_hp; }
    StereoLinkwitzRileyFilter<SampleType>& getAllPass() { return m_ap; }
...
public:
    ...
    void initialize(Slope slope, const SampleType* freqs, size_t numFreqs, SampleType sampleRate) {
        if (numFreqs < 1 || numFreqs > MAX_FREQS) {
            throw std::invalid_argument("MultiBandN requires 1-6 crossover frequencies (2-7 bands)");
        }
     ...
     */
    size_t getNumBands() const { return m_numBands; }
    
    /**
     * Get the crossover frequencies
     */
    const SampleType* getFrequencies() const { return m_freqs.data(); }
     ...
     */
    void processBlock(const SampleType* inL, const SampleType* inR,
                      SampleType** bandsL, SampleType** bandsR, size_t numSamples) {
